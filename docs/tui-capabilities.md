...
| Sandbox mode controls | TUI `/sandbox` and headless `exec --sandbox` share `read-only`, `workspace-write`, and `danger-full-access`; sandbox scope stays independent from approval policy | Shared config projection, runtime capability, CLI validation, and TUI command tests; restricted execution still depends on platform sandbox support |
