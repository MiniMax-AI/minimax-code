...
describe('LocalBashTool — foreground output truncation', () => {
  ...
  it.skipIf(process.platform === 'win32')('reports signal termination as a failed tool result with partial output', async () => {
    const tool = new LocalBashTool(workspace, undefined, { mode: 'off' });
    const result = await tool.execute(SESSION_CTX, {
    ...
    expect(result.text).toContain('Command terminated by signal SIGTERM');
    expect(result.details).toMatchObject({
      execution: { status: 'failed', reason: 'signaled', exitCode: null, signal: 'SIGTERM' },
      processOutput: { stdout: 'partial-output', stderr: '' },
    });
  });

  it('preserves original head and tail with one readable full output file', async () => {
    const tool = new LocalBashTool(workspace, undefined, { mode: 'off' });
    ...
    const full = await readFile(result.details?.fullOutputPath as string, 'utf8');
    expect(full).toContain('row-200-');
    expect(result.details?.output).toMatchObject({ rawBytes: Buffer.byteLength(full), persistence: 'complete' });
  ...
  it('caps a large nonzero-exit result while preserving the error ToolResult status', async () => {
    ...
    expect(result.details).toMatchObject({
      ...
      processOutput: { exitCode: 7, interrupted: false },
    });
    expect(result.text).toContain('desktop bash output truncated');
    expect(result.details?.desktop_output_truncation).toMatchObject({
      truncated: true,
  ...
  it('output beyond 2000 lines retains the original first and last lines', async () => {
    const tool = new LocalBashTool(workspace, undefined, { mode: 'off' });
    ...
    expect(lower).toContain('original head+tail');
    expect(Buffer.byteLength(result.text, 'utf8')).toBeLessThanOrEqual(
      DESKTOP_BASH_MAX_BYTES,
    );
    ...
    expect(fullLines).toHaveLength(3000); // complete output, not the 50 KiB tail
