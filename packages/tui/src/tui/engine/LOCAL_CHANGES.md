...
## L038: Preserve the native scrollback viewport during history refresh

- Product contract: a history refresh or status update must not reset a user who is reading older regular-mode output back to the top of the terminal scrollback.
- Minimal difference: when changed text is already in the immutable native scrollback prefix, `TuiMainScreen` uses its viewport-only redraw path instead of clearing and replaying the complete projection. Structural shrink rebuilds and resize replay remain unchanged.
- Tradeoff: the native scrollback is a stable snapshot until a later structural rebuild; it may temporarily show the previous prefix text while the current projection is kept in renderer state.
- Evidence: `test/unit/tui-engine-local-deltas.test.ts` simulates native mouse scrolling, changes a historical row, and asserts that the visible viewport and scrollback position remain unchanged without emitting `CSI 3 J`.
- Removal condition: the selected Pi baseline provides a native scrollback position anchor for history replay.
